...
func TestProcessNewPod(t *testing.T) {
	...
	listener.filters, _ = setupContainerFilters()
	...
	case service := <-services:
		...
		assert.Equal(t, ErrNotSupported, err)
		assert.Len(t, service.GetCheckNames(), 0)
		assert.False(t, service.IsMetricExcluded())
		assert.False(t, service.IsLogExcluded())
	default:
	...
	case service := <-services:
		...
		assert.Equal(t, ErrNotSupported, err)
		assert.Len(t, service.GetCheckNames(), 0)
		assert.True(t, service.IsMetricExcluded())
		assert.False(t, service.IsLogExcluded())
	default:
	...
	case service := <-services:
		...
		assert.Len(t, service.GetCheckNames(), 0)
		assert.False(t, service.IsMetricExcluded())
		assert.True(t, service.IsLogExcluded())
	default:
